...
Not built: the verified statutory dataset, and the statutory deadline types that fire from events rather than project facts. Stage D, notice drafting, is built: a deterministic template drafter that gets the letter's shape right from the claim file, an Anthropic drafter that writes only the description paragraph, and both policed by the SERVICE rather than by the provider — a legal conclusion anywhere in the body falls back to the template, and a draft that no longer quotes the clause verbatim is refused outright. Stage E, the claim file, is built: assembled as a query from what the job already recorded, rendered as Markdown, gated per instrument rather than per project, and it leads with what it is MISSING rather than burying it. Stage B is now complete: the two-pass profile (heuristic or model screening, then careful extraction over the candidates only), the Anthropic implementation of both passes, the `Read this contract` action on the review screen, and the discard count shown to the reviewer so the citation gate is visible rather than merely present.
